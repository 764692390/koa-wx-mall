  ...
  <div class="ivu-card">
            ...
            <FormItem label="面试时间">
              <div class="cl">
                <div class="fl" style="width:46%">
                  <DatePicker
                    v-model="formTop.InterviewTimer_Start"
                    @on-change="InterviewTimerStart"
                    format="yyyy-MM-dd"
                    type="date"
                    placeholder="选择开始日期"
                    style="width: 100%"
                ...
                <div class="fr" style="width:46%">
                  <DatePicker
                    v-model="formTop.InterviewTimer_End"
                    @on-change="InterviewTimerEnd"
                    format="yyyy-MM-dd"
                    type="date"
                    placeholder="选择开始日期"
                    style="width: 100%"
      ...
      <div class="fulal">
        ...
        <div class="page">
          <Page
            ...
            :current="Math.floor(pageNum)"
            :page-size="Math.floor(pageSize)"
            show-elevator
            show-sizer
            class="pageCou"
            @on-change="getpage"
    ...
    >
      <Form
        ...
        ref="addFrom"
        label-position="top"
        style="padding-left: 3%; padding-top: 1%;"
      >
        <Row>
          <Col span="8">
            <FormItem label="姓名">
          ...
          <Col span="8">
            <FormItem label="面试时间">
              <div class="cl">
                <DatePicker
                  v-model="addFrom.InterviewTimer"
                  @on-change="addFromInterviewTimer"
                  format="yyyy-MM-dd"
                  type="date"
                  placeholder="选择开始日期"
                  style="width: 100%"
          ...
          <Col span="8">
            <FormItem label="入职时间">
              <div class="cl">
                <DatePicker
                  v-model="addFrom.EntryTimer"
                  @on-change="addFromEntryTimer"
                  format="yyyy-MM-dd"
                  type="date"
                  placeholder="选择开始日期"
                  style="width: 100%"
    ...
    <!--详情-->
    <Modal v-model="modal2" title="详情" width="800" :mask-closable="false">
      <Row v-if="modal2" class="datail-box">
        ...
        <Col span="8">
          <i>职位：</i>
          <span>{{ this.jobToName(detailModelData.jobPosition) }}</span>
        </Col>
        <Col span="8">
          <i>是否转正：</i>
...
export default {
    ...
    return {
      ...
      columns1: [
        ...
        },
        {
          ...
          align: 'center',
          render: (h, params) => {
            console.log(this.jobToName())
            return h('div', {}, this.jobToName(params.row.jobPosition))
          }
        },
        {
          title: '面试时间',
          ...
          render: (h, params) => {
            return h(
              'div',
              {},
              getDateToHtml(params.row.InterviewTimer).split(' ')[0]
            )
          }
        },
        {
            ...
            return h('div', [
              h('Icon', {
                attrs: {
                  ...
                  style:
                    'font-size:26px;cursor: pointer; margin-left:8px;margin-top: 4px;'
                },
                on: {
                  click: () => {
                    this.modal2 = true
  ...
  methods: {
    ...
    // 初始化
    async init () {
      await this.getAllJobTypeFn()
      this.getList()
    },
    async getAllJobTypeFn () {
      // 获取所有 职位
      getAllJobType().then(res => {
        console.log(res)
        if (res.data.errno === '0') {
          this.allJobType = res.data.data
        } else {
    ...
    // 新增面试时间
    addFromInterviewTimer (ev) {
      this.addFrom.InterviewTimer = ev
    },
    // 新增入职时间
    addFromEntryTimer (ev) {
      this.addFrom.EntryTimer = ev
    },
    detailSubmit () {},
    handleReset () {},
    // 字典
    jobToName (id) {
      let name = '';
      for (let i = 0; i < this.allJobType.length; i++) {
        if (this.allJobType[i].job_type === id) {
          name = this.allJobType[i].name
        }
      }
      return name
    }
